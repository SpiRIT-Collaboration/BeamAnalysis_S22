  ...
  for(Int_t j=0;j<seg->GetNumData();j++){
    ...
    if(PLAT == detector /*|| STOPPLA == detector*/ || RF == detector ){
      if(mm==*((TArtRIDFMap *)para->GetTLMap())){
        //if(pla->GetTLRaw()<=0){ 
        if(pla->GetTLRaw()<=0 || TMath::Abs(para->GetTLMean()-pla->GetTLRaw())>=TMath::Abs(para->GetTLMean()-val) ){
          pla->SetTLRaw(val);
        }
        else{
      ...
      if(mm==*((TArtRIDFMap *)para->GetTRMap())){
        //if(pla->GetTRRaw()<=0){
        if(pla->GetTRRaw()<=0 || TMath::Abs(para->GetTRMean()-pla->GetTRRaw())>=TMath::Abs(para->GetTRMean()-val) ){
          pla->SetTRRaw(val);
        }
        else{
