  ...
  for(Int_t j=0;j<seg->GetNumData();j++){
    ...
    if(PPACT == detector){
      if(mm == *((TArtRIDFMap *)para->GetTX1Map())){
        //if(ppac->GetTX1Raw()<=0)ppac->SetTX1Raw(val);  // changed to get TDC value that is nearest to its mean from MHTDC 170712, kaneko
        if(ppac->GetTX1Raw()<=0 || TMath::Abs(para->GetTX1Mean()-ppac->GetTX1Raw())>=TMath::Abs(para->GetTX1Mean()-val) ) ppac->SetTX1Raw(val);
        //if(val>100000)ppac->SetTX1Raw(val);
        //if(val>60000) ppac->SetTX1Raw(0);
      }
      else if(mm == *((TArtRIDFMap *)para->GetTX2Map())){
        //if(ppac->GetTX2Raw()<=0)ppac->SetTX2Raw(val);
        if(ppac->GetTX2Raw()<=0 || TMath::Abs(para->GetTX2Mean()-ppac->GetTX2Raw())>=TMath::Abs(para->GetTX2Mean()-val) ) ppac->SetTX2Raw(val);
        //if(val>100000)ppac->SetTX2Raw(val);
        //if(val>60000) ppac->SetTX2Raw(0);
      }
      else if(mm == *((TArtRIDFMap *)para->GetTY1Map())){
        //if(ppac->GetTY1Raw()<=0)ppac->SetTY1Raw(val);
        if(ppac->GetTY1Raw()<=0 || TMath::Abs(para->GetTY1Mean()-ppac->GetTY1Raw())>=TMath::Abs(para->GetTY1Mean()-val) ) ppac->SetTY1Raw(val);
        //if(val>100000)ppac->SetTY1Raw(val);
        //if(val>60000) ppac->SetTY1Raw(0);
      }
      else if(mm == *((TArtRIDFMap *)para->GetTY2Map())){
        //if(ppac->GetTY2Raw()<=0)ppac->SetTY2Raw(val);
        if(ppac->GetTY2Raw()<=0 || TMath::Abs(para->GetTY2Mean()-ppac->GetTY2Raw())>=TMath::Abs(para->GetTY2Mean()-val) ) ppac->SetTY2Raw(val);
        //if(val>100000)ppac->SetTY2Raw(val);
        //if(val>60000) ppac->SetTY2Raw(0);
      }
      else if(mm == *((TArtRIDFMap *)para->GetTAMap())){
        //if(ppac->GetTARaw()<=0)ppac->SetTARaw(val);
        if(ppac->GetTARaw()<=0 || TMath::Abs(para->GetTAMean()-ppac->GetTARaw())>=TMath::Abs(para->GetTAMean()-val) ) ppac->SetTARaw(val);
        //if(val>100000)ppac->SetTARaw(val);
        //if(val>60000) ppac->SetTARaw(0);
      }
    }
